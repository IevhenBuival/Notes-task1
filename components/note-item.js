...
const renderBtnField = function (text, size, root) {
  const noteField = createTag(
    ...
    "col-" +
      size +
      " border rounded border-primary bg-white text-truncate d-flex px-0"
  );
  noteField.innerText = text;
  root.appendChild(noteField);
  return noteField;
...
const appendActionBtn = function (type, root) {
  const commonActionBtnStyle = "border flex-btn";
  const button = createTag("button", type + " " + commonActionBtnStyle);
  if (type === "edit-btn")
    button.innerHTML =
...
const renderNote = function (note, root) {
  const noteRow = createTag(
    "div",
    "note container-fluid row w-100 text-primary rounded"
  );
